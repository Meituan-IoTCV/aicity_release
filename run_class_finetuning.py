...
def get_args():
    ...
    parser.add_argument('--view', default="right", type=str,
                        help='dataset path for evaluation')
...
def main(args, ds_init):
    ...
    if model_ema is not None:
        ema_stats = validation_one_epoch(data_loader_val, model_ema.ema, device)
        print(f"Accuracy of the ema network on the {len(dataset_val)} val videos: {ema_stats['acc1']:.1f}%")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == args.warmup_epochs and model_ema is not None:
            print("==>> Initi model from {} ".format(args.warmup_epochs))
            model_ema = ModelEma(
                model.module,
                decay=args.model_ema_decay,
                device='cpu' if args.model_ema_force_cpu else '',
